...
const AntiGravityCanvas: React.FC = () => {
  ...
  const initParticles = useCallback((width: number, height: number) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      newParticles.push({ x, y, originX: x, originY: y, vx: 0, vy: 0, size: randomRange(1, 2.5), color: Math.random() > 0.9 ? '#4285F4' : '#ffffff', angle: Math.random() * Math.PI * 2 });
    }
  ...
  const animate = useCallback((time: number) => {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (mouse.isActive && distance < MOUSE_RADIUS) {
        ...
        p.vx -= forceDirectionX * repulsion * 5; p.vy -= forceDirectionY * repulsion * 5;
      }
      ...
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (distSq < minDist * minDist) {
          ...
          if (dist > 0.01) {
            ...
            const pushX = nx * overlap * 0.5; const pushY = ny * overlap * 0.5;
            p1.x -= pushX; p1.y -= pushY; p2.x += pushX; p2.y += pushY;
            const dvx = p1.vx - p2.vx; const dvy = p1.vy - p2.vy;
            const velocityAlongNormal = dvx * nx + dvy * ny;
            if (velocityAlongNormal > 0) {
              const m1 = p1.size; const m2 = p2.size; const restitution = 0.85;
              const impulseMagnitude = (-(1 + restitution) * velocityAlongNormal) / (1 / m1 + 1 / m2);
              const impulseX = impulseMagnitude * nx; const impulseY = impulseMagnitude * ny;
              p1.vx += impulseX / m1; p1.vy += impulseY / m1;
              p2.vx -= impulseX / m2; p2.vy -= impulseY / m2;
            }
          }
        }
      }
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 z-0 overflow-hidden bg-black cursor-crosshair">
...
const Navigation: React.FC = () => (
  <nav className="absolute top-0 left-0 w-full z-20 flex justify-end items-center p-6 md:p-8">
    <button className="text-white/80 hover:text-white transition-colors"><Info size={24} /></button>
  </nav>
);
  ...
  <div className="absolute inset-0 z-10 flex flex-col items-center justify-center pointer-events-none px-4">
    <div className="max-w-4xl w-full text-center space-y-8">
      <div className="inline-block animate-fade-in-up">
        <span className="py-1 px-3 border border-white/20 rounded-full text-xs font-mono text-white/60 tracking-widest uppercase bg-white/5 backdrop-blur-sm">Secure Password Generation</span>
      </div>
      <h1 className="text-6xl md:text-8xl lg:text-9xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-white to-white/40 tracking-tighter mix-blend-difference">Safe<br/>Pass</h1>
      <p className="max-w-2xl mx-auto text-lg md:text-xl text-white/60 font-light leading-relaxed">Generate strong, unique passwords in an instant - entirely in your browser. No server, no storage, no compromise. Your security stays yours.</p>
      <div className="pt-8 pointer-events-auto">
        <Link href="/generate" className="group relative inline-flex items-center gap-3 px-8 py-4 bg-white text-black rounded-full font-bold tracking-wide overflow-hidden transition-transform hover:scale-105 active:scale-95">
          <span className="relative z-10">Generate Password</span>
